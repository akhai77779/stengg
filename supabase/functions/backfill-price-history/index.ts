...
// ─── BTC Real Pattern ───────────────────────────────────────────────────────
// Extracted % changes from real BTC/USDT 30m Binance data
// Shape: consolidation → pullback → drop → recovery
// We replay these as 1M candles scaled to each product's volatility
...
// Product configs — volatility scale per symbol
const PRODUCT_CONFIG: Record<string, { basePrice: number; volatility: number; trendTilt: number }> = {
  "AGIL/USDT": { basePrice: 601.94, volatility: 0.008, trendTilt: 0.0003 },
  "360SA/USDT": { basePrice: 40.75, volatility: 0.007, trendTilt: 0.0002 },
  "MCS/USDT": { basePrice: 14.51, volatility: 0.006, trendTilt: 0.0 },
  "SIM/USDT": { basePrice: 9.06, volatility: 0.022, trendTilt: 0.0003 },
  "COTM/USDT": { basePrice: 9.12, volatility: 0.008, trendTilt: 0.0 },
  "IBMS/USDT": { basePrice: 2.69, volatility: 0.014, trendTilt: -0.0003 },
  "VICS/USDT": { basePrice: 1.0, volatility: 0.035, trendTilt: 0.0 },
  "HED/USDT": { basePrice: 6.49, volatility: 0.018, trendTilt: -0.0002 },
  "C5ISR/USDT": { basePrice: 55.2, volatility: 0.009, trendTilt: 0.0 },
  "WIG/USDT": { basePrice: 12.5, volatility: 0.022, trendTilt: 0.0 },
};

const BTC_BASE_VOL = 0.005; // avg volatility per 30m candle in BTC pattern
...
/**
 * Generate 1M candles with REAL timestamps going back `totalMinutes` from now.
 * Uses BTC % pattern scaled to product volatility.
 * Each candle has `recorded_at` = exact minute boundary (UTC).
 */
function generateCandles(symbol: string, totalMinutes: number, startingPrice?: number): GeneratedCandle[] {
  const cfg = PRODUCT_CONFIG[symbol];
  if (!cfg) return [];

  const base = startingPrice && startingPrice > 0 ? startingPrice : cfg.basePrice;
  ...
  // Start offset: different per symbol so charts look different
  const symbolHash = symbol.split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  const startOffset = symbolHash % BTC_PCT.length;

  // Align to exact minute boundary
  ...
  for (let i = totalMinutes; i >= 1; i--) {
    // Exact minute timestamp going back from now
    const minuteMs = nowMinute - i * 60000;
    const recordedAt = new Date(minuteMs).toISOString();

    const idx = (startOffset + (totalMinutes - i)) % BTC_PCT.length;
    const rawPct = BTC_PCT[idx];
    const scaledPct = rawPct * volScale + cfg.trendTilt;

    // Occasional spike (mirrors real market)
    const spike = Math.random() < 0.015 ? 2.0 + Math.random() * 1.5 : 1.0;
    const change = price * scaledPct * spike;

    const open = price;
    let close = open + change;

    // Soft clamp
    if (close < minPrice) close = minPrice + (minPrice - close) * 0.2;
    else if (close > maxPrice) close = maxPrice - (close - maxPrice) * 0.2;
    ...
    const baseVol = (base * 200) / totalMinutes;
    const movePct = Math.abs(scaledPct);
    const volume = baseVol * (0.5 + Math.random() * 0.8) * (1 + movePct / (cfg.volatility * 0.5));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Total 1M candles = days * 24h * 60min
    ...
    for (const product of products) {
      ...
      const cfg = PRODUCT_CONFIG[symbol];
      if (!cfg) {
        results[product.id] = { inserted: 0, error: `No config for symbol: ${symbol}` };
        continue;
      }

      try {
        // Delete existing history for this product to avoid duplicates
        await supabase.from("price_history").delete().eq("product_id", product.id);

        // Use current DB price if available (keeps continuity with live engine)
        const startingPrice = product.price ?? cfg.basePrice;

        // Generate candles with real timestamps
        const candles = generateCandles(symbol, totalMinutes, startingPrice);
        ...
        // Update product current price to last candle
