              ...
              <div className="flex items-end gap-3 md:gap-4 -mt-8 md:-mt-10">
                ...
                <div className="flex-1 pb-2">
                  <h2 className="text-base md:text-lg font-bold text-foreground">
                    {profile?.full_name || (user.email?.endsWith('@phone.local') ? (profile?.phone || `+${user.email.replace('@phone.local', '')}`) : user.email?.split('@')[0]) || t('profile.defaultUser')}
                  </h2>
                  <button onClick={copyUID} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors min-h-[44px] -my-2">
                    <span>ID: {uid}</span>
                    <Copy className="lucide lucide-copy w-3 text-[#23f91f] h-[13px]" />
